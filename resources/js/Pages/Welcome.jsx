    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-gray-100 dark:bg-black text-gray-800 dark:text-gray-200 font-['Inter']">
                {/* Header / Navigasi */}
                <header className="fixed top-0 left-0 right-0 z-40 bg-white/80 dark:bg-black/80 backdrop-blur-sm shadow-sm transition-all duration-300">
                                    ...
                                    <Link href={route('panduan.index')} key={panduan.id}>
                                        <Card className="text-center h-full hover:shadow-xl hover:-translate-y-2 transition-all duration-300 rounded-xl">
                                            <CardHeader>
                                                ...
                                                <CardTitle className="mt-4">{panduan.title}</CardTitle>
                                            </CardHeader>
                                        </Card>
                                    </Link>
                                ))}
                                ...
                                <div className="space-y-8">
                                    {beritas.map(berita => (
                                        <div key={berita.id} className="flex gap-4 items-start">
                                            {berita.image && <img src={`/storage/${berita.image}`} alt={berita.title} className="w-24 h-24 object-cover rounded-lg hidden sm:block"/>}
                                            <div>
                                                ...
                                                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1 line-clamp-2">{berita.summary || berita.body.substring(0, 120) + '...'}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="space-y-4">
                                    {kegiatans.length > 0 ? kegiatans.map(kegiatan => (
                                        <Card key={kegiatan.id}>
                                            <CardContent className="pt-6 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg">
                            ...
                            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                                ...
                                <div className="mt-12 grid grid-cols-2 md:grid-cols-4 gap-4">
                                    {(albumTerbaru.images || []).slice(0, 4).map((imagePath, index) => (
                                        <Link href={route('galeri.show', albumTerbaru.slug)} key={index} className="overflow-hidden rounded-lg group">
                                            <img src={`/storage/${imagePath}`} alt={albumTerbaru.title} className="h-full w-full object-cover aspect-square group-hover:scale-105 transition-transform"/>
                                        </Link>
                                    ))}
                                </div>
